...
<x-atoms.surface
    tag="section"
    aria-labelledby="disbursement-form-title"
    x-cloak
    x-transition
>
    <x-molecules.form-header
        id="disbursement-form-title"
        badgeXText="editingId ? 'Mode edit' : 'Input Baru'"
        titleXText="editingId ? 'Edit Pengeluaran Kas' : 'Tambah Pengeluaran Kas'"
        subtitleXText="editingId ? 'Perbarui detail transaksi kas keluar.' : 'Isi detail transaksi kas keluar. Keterangan dapat diisi otomatis berdasarkan kode transaksi yang dipilih.'"
    />

    <form
        x-ref="disbursementForm"
    ...
    >
        ...
        <x-atoms.input
            ...
            aria-label="Tanggal transaksi pengeluaran"
        />
        ...
        <x-atoms.input
            ...
            required
            placeholder="Masukkan nomor dokumen (contoh: BKKUDD001)"
            aria-label="Nomor dokumen pengeluaran"
        />
        ...
        <x-atoms.input
            ...
            x-model="userId"
            required
            aria-label="Pilih penerima pembayaran"
        >
            <option value="">Pilih penerima / dibayarkan kepada</option>
            @foreach ($users as $u)
        ...
        <x-atoms.input
            ...
            required
            aria-label="Pilih rekening kas yang digunakan"
        >
        ...
        <x-atoms.input
            ...
            required
            aria-label="Pilih kode transaksi pengeluaran"
        ...
        <div>
            <x-atoms.input
                ...
                step="1"
                required
                aria-label="Jumlah nominal pengeluaran"
            />
        ...
        <x-atoms.input
            name="reference"
            label="Referensi"
            placeholder="Nomor faktur, kuitansi, dll. (opsional)"
            x-model="reference"
            maxlength="100"
            aria-label="Nomor referensi transaksi pengeluaran"
        />

        {{-- Keterangan --}}
        <div class="md:col-span-2">
            <div class="mb-2 flex items-center justify-between">
                <label for="disbursement-description" class="block text-xs font-bold uppercase tracking-normal text-content-base">
                    Keterangan
                </label>
                <span
            ...
            <textarea
                ...
                aria-label="Keterangan transaksi pengeluaran"
                aria-describedby="disbursement_description_hint"
            ></textarea>
            <p id="disbursement_description_hint" class="mt-1.5 text-xs text-content-muted">
                Keterangan akan terisi otomatis jika kode transaksi yang dipilih memiliki sugesti.
            </p>
        </div>

        <x-molecules.form-actions
            submitText="Simpan Pengeluaran Kas"
            submitXText="editingId ? 'Simpan Perubahan' : 'Simpan Pengeluaran Kas'"
            ...
            submitAriaLabel="Simpan data pengeluaran kas"
            cancelAriaLabel="Batal edit pengeluaran kas"
        />
    </form>
</x-atoms.surface>
